...
void gameUpdate() {
  ...
  if (currentTime - updateIntervalStartTime >= updateInterval) {
    updateBoard();
    updateIntervalStartTime = currentTime;
  }
}
...
void setTile(int x, int y, boolean active, int frame) {
  for (int i = 0; i < electrodeArray.length; i++) {
    if (electrodeArray[i].x == (x - viewSizeX / 2) * 2 && electrodeArray[i].y == (y - viewSizeY / 2) * 2) {
      fluxels[electrodeArray[i].e][frame] = active;
      break;
    }
  }
}
...
//img.resize(500, 500);
 ...
 if (showFeedbackUI) text("feedback", coX(37),coY(-1.5));  
  ...
  text("reset", coX(37),coY(5.5));  
  text("clear", coX(37),coY(8.5));
